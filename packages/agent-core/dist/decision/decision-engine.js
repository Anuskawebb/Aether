...
import { db, tradeRecommendations, SmartMoneySignalsRepository, TradeRecommendationsRepository, } from '@toro/db';
import { PriceService } from '../valuation/price-service.js';
import { PriceState } from '../valuation/price-types.js';
import { RiskEngine } from '../risk/risk-engine.js';
import { PortfolioStateService } from '../portfolio/portfolio-state-service.js';
import { PositionRegistryService } from '../position/position-registry-service.js';
import { rankOpportunities } from './decision-ranking.js';
...
export class DecisionEngine {
    ...
    async run(now = new Date()) {
        ...
        const signalsByToken = new Map(signals.map(s => [s.tokenAddress, s]));
        const rankInputs = [];
        let blockedCount = 0;
        ...
        for (const opportunity of allocation.approved) {
            recommendations.push(this.buildBuyRecommendation(opportunity, portfolioSnapshot, now));
        }
    ...
    // ── Exit Engine ────────────────────────────────────────────────────────────
    async evaluateExits(openPositions, signalsByToken, now) {
        const sellRecs = [];
    ...
    isSignalReversal(signal) {
        if (signal.trend === 'DECREASING')
            return true;
        if (signal.netAccumulationFlow < 0)
            return true;
        if (signal.netAccumulationFlow24h !== null && signal.netAccumulationFlow24h < 0)
            return true;
        return false;
    }
    // ── Recommendation builders ────────────────────────────────────────────────
    buildBuyRecommendation(opportunity, portfolio, decidedAt) {
        const { signal, marketPriceUsd, convictionScore, expectedEdge, positionSizePct, stopLossPct, takeProfitPct, slippageLimitPct, riskTier } = opportunity;
    ...
    buildSellRecommendation(position, currentPriceUsd, exitReasonDetail, decidedAt) {
        const pnlPct = ((currentPriceUsd - position.entryPriceUsd) / position.entryPriceUsd) * 100;
        const currentValueUsd = Math.round(position.positionSizeUsd * (currentPriceUsd / position.entryPriceUsd) * 100) / 100;
        return {
            id: randomUUID(),
            agentWallet: this.agentWallet,
    ...
    // ── Persistence ────────────────────────────────────────────────────────────
    async persistRecommendations(recs) {
        const toRow = (r) => ({
            id: r.id,
