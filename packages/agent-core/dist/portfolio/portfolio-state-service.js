import { db, walletPositions, portfolioSnapshots, portfolioState, eq, and, asc, gte, lt, } from '@toro/db';
...
import { toRiskPortfolioState, } from './portfolio-types.js';
/**
...
export class PortfolioStateService {
    ...
    async refresh(now = new Date()) {
        ...
        const valuation = PortfolioValuationEngine.compute(this.agentWallet, positions, priceMap, BSC_STABLES, now);
     ...
     */
    async pruneSnapshots(retentionDays = 7, now = new Date()) {
        const cutoff = new Date(now.getTime() - retentionDays * 24 * 60 * 60 * 1000);
        await db
            .delete(portfolioSnapshots)
            .where(and(eq(portfolioSnapshots.agentWallet, this.agentWallet), lt(portfolioSnapshots.snapshotAt, cutoff)));
     ...
     */
    async resolveStoredPeakAndBootstrap() {
        const rows = await db
     ...
     */
    computeDrawdown(currentUsd, peakUsd) {
        if (peakUsd <= 0)
            return 0;
     ...
     */
    async computeRollingLoss(currentUsd, now) {
        const cutoff = new Date(now.getTime() - 24 * 60 * 60 * 1000);
        const windowRows = await db
            .select({ portfolioUsd: portfolioSnapshots.portfolioUsd })
            .from(portfolioSnapshots)
            .where(and(eq(portfolioSnapshots.agentWallet, this.agentWallet), gte(portfolioSnapshots.snapshotAt, cutoff)))
...
//# sourceMappingURL=portfolio-state-service.js.map