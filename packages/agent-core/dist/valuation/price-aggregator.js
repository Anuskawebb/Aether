...
import { PriceState, computeConfidenceBreakdown } from './price-types.js';
...
export class PriceAggregator {
     ...
     */
    static async aggregatePrices(now = new Date(), liquidityOverrides, tokenAddresses) {
        ...
        const latestObsRows = await db.execute(sql `
    ...
    `);
        ...
        const hourObsRows = await db
            ...
            .where(and(inArray(priceObservations.tokenAddress, addressesToProcess), gte(priceObservations.observedAt, oneHourAgo)));
        ...
        const calculateVwap = (obsList, fallback) => {
            if (obsList.length === 0)
                return fallback;
            let sumUsdVolume = 0;
        ...
        for (const tokenAddress of addressesToProcess) {
            ...
            if (!latestObs) {
                ...
                if (existingPrice)
                    unresolvableAddresses.push(tokenAddress);
                continue;
            }
     ...
     */
    static async batchDetermineRouteTypes(tokenAddresses) {
        ...
        const stableDirectRows = await db.execute(sql `
      SELECT DISTINCT
        CASE
    ...
    `);
        ...
        const wbnbRouteRows = await db.execute(sql `
      SELECT DISTINCT
        CASE
    ...
    `);
        ...
        const externalRows = await db.execute(sql `
     ...
     */
    static async pruneObservations(retentionDays = 7, now = new Date()) {
