 ...
 *
 * Called by the on-chain agent platform (JSON_API_AGENT_ID) inside
...
export async function GET(
  ...
  const tradePriceScaled = BigInt(Math.round(Number(row.wmntPrice) * 1e10));
  ...
  return NextResponse.json({
    swap: {
      ...
      price_raw:  Number(row.wmntPrice),  // WMNT price in USDe
      tx_hash:    row.txHash ?? '',
