...
const TokenLogo = ({ symbol }: { symbol: string }) => {
  ...
  let src = '';
  if (sym === 'WMNT' || sym === 'MNT') src = '/token-logos/WMNT.png';
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border/80 rounded-2xl p-6 space-y-6 animate-fade-in-up">
        <div>
          ...
          <p className="text-foreground/40 text-[12px]">Configure your agent&apos;s parameters — it will operate autonomously on Mantle from the moment you deploy.</p>
        </div>
        ...
        <div className="bg-foreground/[0.02] border border-foreground/[0.06] rounded-xl px-4 py-4 space-y-2">
          ...
          {[
            {
              icon: '◉',
              text: `Watch ${leaderAddress.slice(0, 6)}…${leaderAddress.slice(-4)} continuously on Mantle`,
            },
            {
              icon: '⬡',
        ...
        <p className="text-[10px] text-foreground/20 text-center leading-normal">
          Your capital is non-custodial. aUSD is locked in your personal agent contract on Mantle Sepolia (chain 5003). The keeper is authorized only to open and close positions — it cannot withdraw your funds.
        </p>
      </form>
